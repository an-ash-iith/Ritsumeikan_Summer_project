...
#        print(row)
        ...
        if 'car' in c:
            if (cy1<((int(y1+y2)//2)+offset) and  cy1>((int(y1+y2)//2)-offset)) or (cy2<((int(y1+y2)//2)+offset) and  cy2>((int(y1+y2)//2)-offset)) :
...
#                 cv2.putText(frame,str(id),(cx,cy),cv2.FONT_HERSHEY_COMPLEX,0.8,(0,255,255),2)
                if counter.count(id)==0:
                    counter.append(id)
                    distance = 3 # meters
                    a_speed_ms = distance / (elapsed_time/60)
        ...
        if id in vh_up:
            if cy1<(cy+offset) and  cy1>(cy-offset):
                ...
                if counter1.count(id)==0:
                    counter1.append(id)
                    distance1 = 3 # meters
                    a_speed_ms1 = distance / (elapsed_time/180)
